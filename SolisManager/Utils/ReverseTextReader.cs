...
public sealed class ReverseLineReader : IEnumerable<string>
{
    ...
    public ReverseLineReader(string filename, Encoding encoding)
        : this(() => new FileStream(filename, FileMode.Open, FileAccess.ReadWrite, FileShare.ReadWrite), encoding)
    {
    }
    ...
    private IEnumerator<string> GetEnumeratorImpl(Stream stream)
    {
        try
        {
            ...
            while ( position > 0 )
            {
                ...
                var charsRead = encoding.GetChars(buffer, firstCharPosition, bytesToRead - firstCharPosition,
                    charBuffer, 0);
